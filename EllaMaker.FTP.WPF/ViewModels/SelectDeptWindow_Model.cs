...
namespace EllaMaker.FTP.ViewModels
{
    ...
    public class SelectDeptWindow_Model : ViewModelBase<SelectDeptWindow_Model>
    {
        // If you have install the code sniplets, use "propvm + [tab] +[tab]" create a property propcmd for command
        // 如果您已经安装了 MVVMSidekick 代码片段，请用 propvm +tab +tab 输入属性 propcmd 输入命令

        public SelectDeptWindow_Model()
        {
            if (IsInDesignMode)
            {
        ...
        static Func<BindableBase, ValueContainer<bool>> _IsFromShareLocator = RegisterContainerLocator<bool>("IsFromShare", model => model.Initialize("IsFromShare", ref model._IsFromShare, ref _IsFromShareLocator, _IsFromShareDefaultValueFactory));
        ...
        protected Property<ObservableCollection<Model.PsAndDeptTreeNodeItem>> _TreeSource = new Property<ObservableCollection<Model.PsAndDeptTreeNodeItem>> { LocatorFunc = _TreeSourceLocator };
        static Func<BindableBase, ValueContainer<ObservableCollection<Model.PsAndDeptTreeNodeItem>>> _TreeSourceLocator = RegisterContainerLocator<ObservableCollection<Model.PsAndDeptTreeNodeItem>>("TreeSource", model => model.Initialize("TreeSource", ref model._TreeSource, ref _TreeSourceLocator, _TreeSourceDefaultValueFactory));
        static Func<ObservableCollection<Model.PsAndDeptTreeNodeItem>> _TreeSourceDefaultValueFactory = () => new ObservableCollection<PsAndDeptTreeNodeItem>();
        #endregion
            ...
            model =>
            {
                ...
                cmd.DoExecuteUITask(
                        ...
                        async e =>
                        {
                            ...
                            string EventRouterName = "SelectDeptResFromSyncEventRouter";
                            if (vm.IsFromShare)
                                EventRouterName= "SelectDeptResFromShareEventRouter";
                            vm.GlobalEventRouter.GetEventChannel(typeof(MesWindowResModel)).RaiseEvent(vm, EventRouterName, new MesWindowResModel()
                            {
                                IsOk = para,
                                WinType = MesWinType.DeptSelectWin,
